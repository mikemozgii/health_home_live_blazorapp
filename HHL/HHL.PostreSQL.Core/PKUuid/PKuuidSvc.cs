...
namespace HHL.PostreSQL.Core.Uuid
{
    public class PKuuidSvc
    {
        ...
        //todo thread-safe
        public ConcurrentQueue<Guid> HHLGuidGuidsCollection { get; set; } = new ConcurrentQueue<Guid>();

        public PKuuidSvc()
        {
            var ids = GenereateHHLGuid(1000);
            foreach (var id in ids)
            {
                HHLGuidGuidsCollection.Enqueue(id);
            }
        }

        public  PKuuidSvc CreateInstance() => new PKuuidSvc();

        public  Guid GenerateGuid() => new PKuuidSvc().GenereateHHLGuid();

        public void Generate_HHLGuidGuidsCollection()
        {
            var ids = GenereateHHLGuid(1000);
            foreach (var id in ids)
            {
        ...
        public string[] GenereateHHLUuid(int qnt)
        {
            var uuids = new string[qnt];
            var results = GenereateHHLGuid(qnt);
            for (var i = 0; i < qnt; i++)
            {
        ...
        public string GenereateHHLUuid()
        {
            return GenereateHHLUuid(1)[0];
        }

        public Guid GenereateHHLGuid()
        {
            if (HHLGuidGuidsCollection.TryDequeue(out var r))
            {
                return r;
            }

            Generate_HHLGuidGuidsCollection();
            return GenereateHHLGuid();
        }

        public Guid[] GenereateHHLGuid(int qnt)
        {
